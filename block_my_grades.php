...
class block_my_grades extends block_base {
    ...
    public function get_content() {
        ...
        if (is_array($newdata)) {
            if (count($newdata) > 0) {
                $newtext = "<table class=\"grades\"><tr><th>".get_string('gradetblheader_course', 'block_my_grades')."</th><th>".
                    get_string('gradetblheader_grade', 'block_my_grades')."</th></tr>";
    ...
    public function grade_data($report) {
        ...
        if ($courses = enrol_get_users_courses($report->user->id, false, 'id, shortname, showgrades')) {
            ...
            foreach ($courses as $course) {
                ...
                $courselink = html_writer::link(new moodle_url('/grade/report/user/index.php',
                    array('id' => $course->id, 'userid' => $report->user->id)), $courseshortname);
                ...
                if (!$canviewhidden and !is_null($finalgrade)) {
                    if ($coursegrade->is_hidden()) {
                        ...
                        $finalgrade = self::blank_hidden_total($report, $course->id, $courseitem, $finalgrade);
                    }
                }
     ...
     * @param string $finalgrade the grade for the courseitem
     * @return string The new final grade
     */
    public function blank_hidden_total($report, $courseid, $courseitem, $finalgrade) {
        global $CFG, $DB;
        static $hidingaffected = null; // Array of items in this course affected by hiding.
        ...
        if ($previoususerid != $report->user->id || $previouscourseid != $courseid) {
            $hidingaffected = null;
            $previoususerid = $report->user->id;
        ...
        if (!$hidingaffected) {
            ...
            foreach ($items as $itemid => $unused) {
                if (!isset($grades[$itemid])) {
                    ...
                    $gradegrade->userid = $report->user->id;
        ...
        if (array_key_exists($courseitem->id, $hidingaffected['altered'])) {
        ...
        } else if (!empty($hidingaffected['unknown'][$courseitem->id])) {
            ...
            if (!$report->showtotalsifcontainhidden[$courseid]) {
                ...
                // Use reprocessed marks that exclude hidden items.
                $finalgrade = $hidingaffected['unknown'][$courseitem->id];
            }
        }
